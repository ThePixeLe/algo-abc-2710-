...
// Étape 2 : Prendre une décision basée sur cette information
SI age < 0 ALORS
...
Maintenant, tu as le plan. Ton travail est de le traduire dans le langage de programmation de ton choix (Python, JavaScript, C++, etc.). Chaque langage a une syntaxe légèrement différente pour les instructions `if`, `else if` (`elif` en Python), et `else`, ainsi que pour lire l'entrée utilisateur et afficher un message.
...
*   Cherche la syntaxe exacte pour `if`, `else if`, et `else`.
